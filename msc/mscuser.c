...
extern const uint8_t BootSect[];

uint8_t  Memory[MSC_MAX_PACKET]; 
void translate_rd( uint32_t offset, uint32_t length);
void translate_wr( uint32_t offset, uint32_t length);
...
void MSC_MemoryRead (void) {
  ...
  translate_rd(Offset,n);
  USB_WriteEP(MSC_EP_IN, &Memory[0], n);
  Offset += n;
  Length -= n;
  ...
  firmware = (uint8_t *)USER_FLASH_START;
    ...
    if (offset < BOOT_SECT_SIZE)
	{
	  /* Can't write boot sector */
		 ...
		 {
		   write_flash((uint32_t *)(firmware + (offset - (BOOT_SECT_SIZE + FAT_SIZE + ROOT_DIR_SIZE))),(uint8_t*)&BulkBuf[0],length);
		 }
}
